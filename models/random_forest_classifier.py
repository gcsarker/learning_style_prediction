...
def train(training_data, validation_data = None, random_state = 2025):
    ...
    rf_classifier = OneVsRestClassifier(RandomForestClassifier(n_estimators=5, random_state=random_state))
    rf_classifier.fit(X_train, y_cls_train)
    print(rf_classifier.get_params())
    
    # Save the model
    joblib.dump(rf_classifier, os.path.join(checkpoint_path,"random_forest_classifier.pkl"))
...
def evaluate(test_data, scaler_X, scaler_y, class_names, checkpoint_path = 'checkpoints'):
  ...
  rf_classifier = joblib.load(os.path.join(checkpoint_path,"random_forest_classifier.pkl"))

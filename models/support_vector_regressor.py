...
def train(training_data, validation_data=None, random_state=2025):
    ...
    # Initialize SVR for each output (4 outputs)
    print("Loading Support Vector Regressor")
    svr_models = []
    for i in range(y_rgr_train.shape[1]):  # Iterate through each output column
        svr = SVR(C=0.5, kernel='rbf')  # You can adjust parameters
        svr.fit(X_train, y_rgr_train[:, i])  # Train on one output at a time (NumPy indexing)
        svr_models.append(svr)

    # Save the models
    joblib.dump(svr_models, os.path.join(checkpoint_path, "svr.pkl"))
...
def evaluate(test_data, scaler_X, scaler_y, class_names, checkpoint_path='checkpoints'):
    ...
    # Load the models
    svr_models = joblib.load(os.path.join(checkpoint_path, "svr.pkl"))

    # Make predictions for each output
    y_pred_rgr_scaled = np.zeros((X_test_scaled.shape[0], len(svr_models)))  # Initialize prediction array
    for i, svr in enumerate(svr_models):
        y_pred_rgr_scaled[:, i] = svr.predict(X_test_scaled)
